...
require('lazy').setup({
  ...
  { 'Bilal2453/luvit-meta', lazy = true },
  {
    ...
    config = function()
      ...
      -- Server-specific config. Anything else uses Mason + lspconfig defaults.
      --  See :h lspconfig-all for available servers and :h vim.lsp.config for keys.
      local servers = {
        tailwindcss = {
      ...
      -- Broadcast blink.cmp's enhanced capabilities to every server
      vim.lsp.config('*', {
      ...
      -- mason-lspconfig 2.x: automatic_enable calls vim.lsp.enable on each
      -- installed server, picking up the configs registered above.
  ...
  { -- Autocompletion
    'saghen/blink.cmp',
    event = 'InsertEnter',
    version = '1.*', -- use a tagged release so the prebuilt rust binary is downloaded
    ---@module 'blink.cmp'
